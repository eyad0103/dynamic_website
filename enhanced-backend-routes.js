...
router.post('/api/save-api-key', (req, res) => {
    try {
        ...
        // Store API key securely (in-memory for now, use database in production)
        if (!global.storedAPIKeys) {
            global.storedAPIKeys = new Map();
        }
        
        global.storedAPIKeys.set('openrouter', apiKey);
...
router.get('/api/get-api-key', (req, res) => {
    try {
        if (!global.storedAPIKeys || !global.storedAPIKeys.has('openrouter')) {
            return res.json({
                success: false,
        ...
        const apiKey = global.storedAPIKeys.get('openrouter');
